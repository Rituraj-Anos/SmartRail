...
class SectionGraph:
    ...
    def find_time_expanded_path(
        ...
        def heuristic(node: str) -> float:
            try:
                return nx.shortest_path_length(
                    self.graph, source=node, target=target, weight="weight"
                )
            except nx.NetworkXNoPath:
                return float("inf")
        ...
        heapq.heappush(
            open_set,
            (heuristic(source), start_time_min, source, [(source, start_time_min)]),
        )
        ...
        while open_set:
            ...
            if (current_node, wait_time) not in visited:
                heapq.heappush(
                    open_set,
                    (
                        wait_time + heuristic(current_node),
                        wait_time,
                        current_node,
                        path + [(current_node, wait_time)],
                    ),
                )
